...
def shelly_switch(motor, on):
    ...
    base_url = str(shelly.get("baseUrl", "")).rstrip("/")
    ...
    def do_GET(self):
        ...
        with lock:
            ...
            if path == "/api/mix/status":
                send_json(self, 200, {**state["mix"], "currentWeight": state["lastWeight"], "motors": state["motors"], "hardware": state["hardware"]})
                return
...
if __name__ == "__main__":
    ...
    print(f"Shelly simulation: {CONFIG.get('shelly', {}).get('simulation')}")
